...
<p align="center">
  <img src="assets/frontier.png" width="92%" alt="Trained solve rate vs difficulty, by board size"/>
</p>
...
0% except for a few percent on near-complete 4×4 boards. The chart above is the full
frontier; the table below is a digest (full version in
...
Per-size curricula: 4×4 `exp1`, 6×6 `exp3`+`exp8`, 8×8 `exp4`–`exp7` (after the
transcription warm-up; 6×6 measured from 2 empty cells). Every size declines to ~0% at
the minimal-clue end.

8×8 took several runs to reach this frontier; each one extended the solvable range:

<p align="center"><img src="assets/frontier_8x8.png" width="88%" alt="8x8 frontier advancing across runs"/></p>

Figures and tables are generated by `scripts/frontier.py` (consolidated charts),
`scripts/plot.py` (per-experiment charts), and `scripts/log_experiment.py`. Per-run
detail is in `experiments/<id>/` and [`experiments/REGISTRY.md`](experiments/REGISTRY.md);
the full narrative is in [`docs/EXPERIMENTS.md`](docs/EXPERIMENTS.md).
...
scripts/
  ...
  plot.py             per-experiment charts
  frontier.py         consolidated cross-experiment frontier charts + table
